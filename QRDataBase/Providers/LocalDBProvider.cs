...
public class LocalDBProvider : IDataBaseProvider
{
    ...
    public List<T> Get<T>(ISearchItem? search = null, int limit = -1)
    {
        ...
        if (search is not DbKeyValue dbKeyValue) return new List<T>();
    ...
    {
        if (data == null) return;
        foreach (var prop in data.GetType().GetProperties()) Console.WriteLine(prop.Name + " = " + prop.GetValue(data));
    }
